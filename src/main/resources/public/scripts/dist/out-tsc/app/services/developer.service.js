...
export var DeveloperService = (function () {
    function DeveloperService(http, utilService, store) {
        this.http = http;
    ...
    DeveloperService.prototype.getDevelopersByLandmarkId = function (cityId, searchString, landmarkId) {
        if (this.utilService.isNull(searchString)) {
            searchString = 'XXXXX';
        }
        var url = this.BASE_URL + "/developer/search/byLandmark/" + landmarkId + "/" + cityId + "/" + searchString;
        return this.http.get(url)
            .map(this.extractData);
    };
